...
import bgu.spl.spl_assignment3.protocol.*;
...
public class ConnectionHandler<T>{
	...
	static private Map<String, Room> rooms;
	static private Map<ProtocolCallback<TBGPMessage>, ClientDetails> clients;
	...
	private ConnectionHandler(SocketChannel sChannel, ReactorData<T> data, SelectionKey key) {
		...
		_protocol = _data.getProtocolMaker().create(clients, rooms);
